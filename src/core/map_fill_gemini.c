...
/* Лимит вершин для аппаратной защиты стека от переполнения */
#define PURRGO_MAP_MAX_POLY_POINTS 128
 ...
 *
 * Параметр is_polygon_layer определяет логику обработки массива точек: 
 * накопление полигонов и заливка (true) или отрисовка полилиний (false).
 */
static void parse_geometry_mlp(
    ...
    // Статический буфер для накопления вершин полигона перед заливкой
    gfx_point_t poly_buf[PURRGO_MAP_MAX_POLY_POINTS];
    ...
    for (uint32_t i = 0; i < (uint32_t)num_points; i++) {
        ...
        if (i == next_part_start) {
            ...
            poly_count = 0; // Сброс счетчика вершин для новой части
        }

        uint32_t current_part_offset = (num_parts > 0) ? parts[current_part_idx] : 0;

        if (is_polygon_layer) {
            // Накопление вершин с защитой границ статического буфера
            if (poly_count < PURRGO_MAP_MAX_POLY_POINTS) {
                poly_buf[poly_count].x = sx;
                poly_buf[poly_count].y = sy;
                poly_count++;
            }
...
static void parse_node(
    ...
    *current_idx_offset += 28;
    ...
    for (uint32_t i = 0; i < obj_count; i++) {
        parse_node(
            idx_fs,
            current_idx_offset, /* Исправлено: передача адреса без лишнего амперсанда */
            mlp_fs,
            child_is_nav,
            child_level,
            cam,
