              ...
              gfx_read_pixel_fn read_pixel_cb)
{
    ...
    /*
     * Быстрый clear callback устанавливается отдельно платформой.
     *
     * По умолчанию NULL означает использование универсального
     * pixel-by-pixel fallback в gfx_clear().
     */
    ctx->clear = NULL;
...
void gfx_set_clear_callback(gfx_context_t *ctx, gfx_clear_fn clear_cb)
{
    if (ctx == NULL) {
        return;
    }
...
{
    if (ctx == NULL || ctx->draw_pixel == NULL) {
        return;
    }

    /*
     * Software clipping against the current clipping region.
     */
    if (x >= ctx->clip_x &&
        x < ctx->clip_x + ctx->clip_w &&
        y >= ctx->clip_y &&
        y < ctx->clip_y + ctx->clip_h) {
        ctx->draw_pixel(
            ctx->framebuffer,
            x,
            y,
...
void gfx_clear(gfx_context_t *ctx)
{
     ...
     * Preferred path:
     *
     * Platform provides a direct framebuffer clear operation.
     *
     * For STM32 this is implemented with memset(), so clearing
     * the 2-bpp framebuffer does not require one callback per pixel.
     */
    if (ctx->clear != NULL) {
        ctx->clear(ctx->framebuffer, ctx->color_bg);
        return;
    }

    /*
     * Generic fallback for platforms without a fast clear operation.
     *
     * This preserves the old behavior and therefore keeps the
...
void gfx_draw_hline(gfx_context_t *ctx, int16_t x_start, int16_t x_end, int16_t y)
{
     ...
     */
    if (x_end < ctx->clip_x ||
        x_start >= ctx->clip_x + ctx->clip_w) {
        return;
    }

    if (x_start < ctx->clip_x) {
        x_start = ctx->clip_x;
    }

    if (x_end >= ctx->clip_x + ctx->clip_w) {
        x_end = ctx->clip_x + ctx->clip_w - 1;
    }

    /*
     * Прямой вызов платформенного callback.
     */
...
void gfx_draw_vline(gfx_context_t *ctx, int16_t x, int16_t y_start, int16_t y_end)
{
     ...
     */
    if (y_end < ctx->clip_y ||
        y_start >= ctx->clip_y + ctx->clip_h) {
        return;
    }

    if (y_start < ctx->clip_y) {
        y_start = ctx->clip_y;
    }

    if (y_end >= ctx->clip_y + ctx->clip_h) {
        y_end = ctx->clip_y + ctx->clip_h - 1;
    }
...
{
    if (ctx == NULL || ctx->read_pixel == NULL) {
        return 0;
    }
