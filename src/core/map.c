...
#include "map_idx.h"
#include "map_render.h" // <--- ДОБАВЛЕНО для map_render_clear_labels()
...
{
    if (idx_fs == NULL || gfx == NULL || camera == NULL || viewport == NULL) return;

    map_diag_t diag = {0};
    uint32_t current_idx_offset = 0;

    pgo_header_info_t idx_header = {0};
    if (!map_parse_pgo_header(idx_fs, &idx_header)) return;
    if (idx_header.file_type != 1) return;

    if (layer_type != MAP_LAYER_POIS) {
        if (mlp_fs == NULL) return;
        pgo_header_info_t mlp_header = {0};
        if (!map_parse_pgo_header(mlp_fs, &mlp_header)) return;
        if (mlp_header.file_type != 2) return;
    }
    ...
    if (!idx_fs->seek(idx_fs->handle, target_lod_offset)) return;
    current_idx_offset = target_lod_offset;

    if (current_idx_offset + 16 > lod_end) return;

    uint8_t sqt_header[16];
    if (idx_fs->read(idx_fs->handle, sqt_header, sizeof(sqt_header)) != sizeof(sqt_header)) return;
    ...
    if (count > 0) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            if (!map_idx_parse_node(idx_fs, &current_idx_offset, mlp_fs, db_fs, is_nav, camera, viewport, gfx, layer_type, &diag, lod_end)) {
                return;
            }
        }
    }
...
{
    // Очищаем кэш BBox-меток для Деклаттеринга перед новым кадром
    ...
    snprintf(landuse_db_path,  sizeof(landuse_db_path),  "%s/landuse.db",  map_dir);
    ...
    purrgo_file_t* landuse_db_file  = purrgo_fs_open(landuse_db_path,  FS_READ);

    bool landuse_success = false;

    if (landuse_idx_file && landuse_mlp_file) {
        purrgo_fs_t landuse_idx_fs = { .handle = landuse_idx_file, .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };
        purrgo_fs_t landuse_mlp_fs = { .handle = landuse_mlp_file, .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };
        purrgo_fs_t landuse_db_fs  = { .handle = landuse_db_file,  .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };

        gfx_set_color(gfx, 2, 3);
        purrgo_map_render_layer(&landuse_idx_fs, &landuse_mlp_fs, landuse_db_file ? &landuse_db_fs : NULL, gfx, camera, viewport, MAP_LAYER_POLYGONS);
    ...
    if (landuse_db_file)  purrgo_fs_close(landuse_db_file);
    ...
    if (idx_file && mlp_file) {
        purrgo_fs_t idx_fs = { .handle = idx_file, .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };
        purrgo_fs_t mlp_fs = { .handle = mlp_file, .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };

        gfx_set_color(gfx, 1, 3);
        // Дороги не подписываем — передаем NULL вместо db_fs
        purrgo_map_render_layer(&idx_fs, &mlp_fs, NULL, gfx, camera, viewport, MAP_LAYER_LINES);
        roads_success = true;
    }
    ...
    /* ------------------- POIS ------------------- */
    snprintf(poi_idx_path, sizeof(poi_idx_path), "%s/pois.idx", map_dir);
    snprintf(poi_db_path,  sizeof(poi_db_path),  "%s/pois.db",  map_dir);

    purrgo_file_t* poi_idx_file = purrgo_fs_open(poi_idx_path, FS_READ);
    purrgo_file_t* poi_db_file  = purrgo_fs_open(poi_db_path,  FS_READ);

    bool poi_success = true;

    if (poi_idx_file) {
        purrgo_fs_t poi_idx_fs = { .handle = poi_idx_file, .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };
        purrgo_fs_t poi_db_fs  = { .handle = poi_db_file,  .read = core_fs_read_wrapper, .seek = core_fs_seek_wrapper };

        purrgo_map_render_layer(&poi_idx_fs, NULL, poi_db_file ? &poi_db_fs : NULL, gfx, camera, viewport, MAP_LAYER_POIS);
    }

    if (poi_idx_file) purrgo_fs_close(poi_idx_file);
    if (poi_db_file)  purrgo_fs_close(poi_db_file);
