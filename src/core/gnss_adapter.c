...
void purrgo_gnss_reset_solution(void) {
    memset(&authoritative_solution, 0, sizeof(authoritative_solution));
}
    ...
    switch (minmea_sentence_id(nmea_line, false)) {
        case MINMEA_SENTENCE_RMC: {
            ...
            if (minmea_parse_rmc(&frame, nmea_line)) {
                authoritative_solution.valid = frame.valid;
                if (frame.valid) {
                    authoritative_solution.lat_1e7 = convert_to_1e7(frame.latitude);
                    authoritative_solution.lon_1e7 = convert_to_1e7(frame.longitude);
                    
                    // Масштабирование скорости (узлы * 100)
                    if (frame.speed.scale != 0) {
                        authoritative_solution.speed_knots = (int32_t)(((int64_t)frame.speed.value * 100) / frame.speed.scale);
                    }

                    // Масштабирование курса (градусы * 100)
                    if (frame.course.scale != 0) {
                        authoritative_solution.course_valid = true;
                        authoritative_solution.course_deg_100 = (int32_t)(((int64_t)frame.course.value * 100) / frame.course.scale);
                    } else {
                        authoritative_solution.course_valid = false;
                    }
                    
                    authoritative_solution.hours = frame.time.hours;
                    authoritative_solution.minutes = frame.time.minutes;
                    authoritative_solution.seconds = frame.time.seconds;
                    authoritative_solution.day = frame.date.day;
                    authoritative_solution.month = frame.date.month;
                    authoritative_solution.year = frame.date.year;
                }
            }
            break;
        }
        case MINMEA_SENTENCE_GGA: {
            ...
            if (minmea_parse_gga(&frame, nmea_line)) {
                authoritative_solution.satellites_tracked = frame.satellites_tracked;
                if (frame.altitude.scale != 0) {
                    authoritative_solution.alt_m = frame.altitude.value / frame.altitude.scale;
                }
                authoritative_solution.fix_quality = frame.fix_quality;
                if (frame.hdop.scale != 0) {
                    authoritative_solution.hdop_100 = (int32_t)(((int64_t)frame.hdop.value * 100) / frame.hdop.scale);
                }
            }
            break;
        }
        case MINMEA_SENTENCE_GSA: {
            ...
            if (minmea_parse_gsa(&frame, nmea_line)) {
                authoritative_solution.fix_type = frame.fix_type;
                if (frame.hdop.scale != 0) {
                    authoritative_solution.hdop_100 = (int32_t)(((int64_t)frame.hdop.value * 100) / frame.hdop.scale);
                }
                if (frame.vdop.scale != 0) {
                    authoritative_solution.vdop_100 = (int32_t)(((int64_t)frame.vdop.value * 100) / frame.vdop.scale);
                }
                if (frame.pdop.scale != 0) {
                    authoritative_solution.pdop_100 = (int32_t)(((int64_t)frame.pdop.value * 100) / frame.pdop.scale);
                }
            }
            break;
        }
