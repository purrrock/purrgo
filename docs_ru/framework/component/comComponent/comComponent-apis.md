...
# Встроенный интерфейс компонента

Фреймворк Glyphix имеет некоторые встроенные свойства для компонентов, доступ к которым осуществляется с использованием формата this.$xxx. Эти встроенные свойства предоставляют компонентам некоторую функциональность за пределами реактивной среды.
...
Доступ к объекту приложения, экспортированному в app.js, можно получить через атрибут $app.

### `$page` <decl type="Component" get />

Доступ к объекту компонента страницы, которой принадлежит компонент, можно получить через свойство $page. Для компонентов страницы значение `this.$page` равно `this`.
