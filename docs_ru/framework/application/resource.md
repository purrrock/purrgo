...
В отличие от этого, `internal://mass/` — это общедоступная область хранения файлов, совместно используемая всеми приложениями. Один и тот же «внутренний» URI указывает на один и тот же файл в разных приложениях. Таким образом, файлы в каталоге «mass» могут быть доступны и совместно использоваться несколькими приложениями. Например, и приложение A, и приложение B используют:
```
внутренний://масс/публичный/shared_image.png
```
Тогда URI указывает на один и тот же общий файл «shared_image.png» в обоих приложениях, что позволяет им совместно использовать файловый ресурс.

::: предупреждение
Если приложение хранит конфиденциальные данные в массовом пространстве, другие приложения могут прочитать эти данные. Поэтому разработчикам следует избегать хранения любой конфиденциальной или частной информации в «массовом» каталоге и гарантировать, что файлы, хранящиеся там, являются общедоступными и общими ресурсами.
:::

## API ресурсов

Глобальные функции [`URI`](/api/global.md#uri), [`@system.path`](/api/system-path.md), [`@system.file`](/api/system-file.md) и другие интерфейсы предоставляют возможность управлять ресурсами в JavaScript. Подробную информацию см. в соответствующей документации.
