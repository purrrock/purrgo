...
`aes` encryption and decryption. The functions of each field in the `options` parameter are:
- `action`: The type of encryption/decryption. Two optional values: `'encrypt'`: encryption, `'decrypt'`: decryption;
- `text`: The text content to be encrypted or decrypted. The text to be encrypted should be plain text, and the text to be decrypted should be a binary value encoded in `base64`;
- `key`: The key used for encryption or decryption, a string generated after `base64` encoding. Before `base64` decoding, the key must be a multiple of $16$ bytes;
- `transformation`: The encryption mode (`'ECB'`, `'CBC'`, `'CFB'`, `'CTR'`, `'OFB'`) and padding of the `AES` algorithm. The default is `'AES/CBC/PKCS5Padding'`. Optional AES padding items are:
  - `'PKCS5Padding'`
  - `'PKCS7Padding'`
  - `'NoPadding'`
  - `'OneAndZerosPadding'`
  - `'ZerosAndLenPadding'`
  - `'ZerosPadding'` 
- `iv`: The initialization vector for AES encryption and decryption, a string encoded in Base64. The default value is the value of the `key` field;
- `ivOffset`: The offset of the initialization vector for AES encryption and decryption. The default value is $0$;
- `ivLen`: The byte length of the initialization vector for AES encryption and decryption. The default value is $16$;
...
- `action`: The type of encryption/decryption, with two optional values: `'encrypt'`: encryption, `'decrypt'`: decryption;
- `text`: The text content to be encrypted or decrypted. The text content to be decrypted should be a segment of binary values encoded in Base64;
- `key`: `RSA` key, a string generated after `base64` encoding. When encrypting, `key` is the public key; when decrypting, `key` is the private key;
- `transformation`: The padding scheme for the RSA algorithm, defaulting to `RSA/None/OAEPwithSHA-256andMGF1Padding`. Optional RSA padding schemes are:
...
rsaTest() // Print the encrypted and decrypted text, console output example
...
`sign` signature, the functions of each field in the `options` parameter are:
- `text`: Content to be signed;
- `key`: RSA private key;
- `algorithm`: Signature algorithm, defaults to `'SHA256withRSA'`. Available signature algorithms are:
  - `'MD5withRSA'`
  - `'SHA1withRSA'`
  - `'SHA256withRSA'`
  - `'SHA512withRSA'`
...
`hash` encryption. The functions of the fields in the `options` parameter are:
- `data`: Original data to generate the digest;
- `algorithm`: Digest algorithm. Optional values: `'md5'`, `'sha1'`, `'sha224'`, `'sha256'`, `'sha384'`, `'sha512'`;
- `encode`: Encoding and type of the returned data. Values:
  - `'hex'`: Default value, returns a hex-encoded string;
  - `'base64'`: The return value is a Base64-encoded string of the encryption result;
...
md5Test() // Print the generated digest to the console
...
Use the HMAC algorithm to generate a keyed-hash message authentication code. The functions of each field in the `options` parameter are:
- `data`: Original data for generating the digest;
- `algorithm`: Digest algorithm, options include `'md5'`, `'sha1'`, `'sha224'`, `'sha256'`, `'sha384'`, `'sha512'`;
- `key`: Secret key;
- `encode`: Encoding and type of the returned data, values include:
  - `'hex'`: Default value, returns a hex-encoded string;
  - `'base64'`: The return value is a Base64-encoded string of the encryption result;
...
hmacTest() // Print the generated digest, output to the console
...
Decodes the input data from Base64.
