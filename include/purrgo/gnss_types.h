...
//
// GNSS Contract:
// - `valid` indicates if a navigation solution exists and coordinates (lat/lon) are populated.
//   It is derived directly from the 'A' (Active) status in RMC frames.
// - Other fields (like alt_m, speed, hdop) are merged continuously from available NMEA sentences (RMC, GGA, GSA).
//   If a specific sentence omits optional data, the previous value is retained.
// - If the parser state is reset or no valid fix has ever been obtained, these fields may contain 0.
//   0 should NOT be considered a safe indicator of invalidity, except for `fix_quality` and `fix_type`.
...
typedef struct {
    ...
    // Дополнительные поля
    int32_t course_deg_100; // Истинный курс (Course over ground) в градусах * 100 (из RMC)
    ...
    uint8_t fix_quality;    // Качество фикса (из $GPGGA: 0=Invalid, 1=GPS, 2=DGPS)
    uint8_t fix_type;       // Тип фикса (из $GPGSA: 1=Нет фикса, 2=2D, 3=3D)
