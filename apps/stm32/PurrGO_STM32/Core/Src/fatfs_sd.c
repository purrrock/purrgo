...
/* Dummy-буфер для удержания MOSI в состоянии HIGH при чтении (спецификация SD-карт) */
static const uint8_t dummy_tx_ff[512] = { [0 ... 511] = 0xFF };
...
{
    // Готово для замены на HAL_SPI_Transmit_DMA
...
{
    // Готово для замены на HAL_SPI_TransmitReceive_DMA
    // Используем dummy_tx_ff для удержания линии MOSI в 1 согласно SD spec
    if (len <= 512) {
        HAL_SPI_TransmitReceive(HSPI_SDCARD, (uint8_t*)dummy_tx_ff, buffer, len, SPI_TIMEOUT);
    } else {
        // Fallback для нестандартных размеров, если такие появятся
        while (len--) {
            *buffer++ = SPI_RxByte();
        }
    }
...
{
    /* Ensure SPI is not transmitting and CS is inactive */
