...
- [ ] Спроектировать собственный global header PurrGO.
  - Использовать собственную сигнатуру PurrGO, например `PGO`.
  - Добавить версию формата.
  - Добавить только необходимые навигатору поля.
  - При необходимости добавить размер payload.
  - При необходимости добавить offset первого SQT.
  - Определить endianess.
  - Определить точный размер каждого поля.
  - Не сохранять DT G1-specific поля без необходимости.

## 2.2. Спецификация header

- [ ] Зафиксировать точный размер header.
- [ ] Зафиксировать offsets.
- [ ] Зафиксировать размеры всех полей.
- [ ] Зафиксировать допустимые значения версии.
- [ ] Определить поведение C-parser при неизвестной версии.
- [ ] Исключить все скрытые зависимости от старого `32-byte YZL header`.
- [ ] Убрать MD5.
...
- [ ] Определить окончательный размер feature code.
- [ ] Проверить, помещается ли полный диапазон PurrGO feature codes в `uint8_t`.
- [ ] Зафиксировать допустимый диапазон.
- [ ] Зарезервировать диапазон для будущих расширений.
- [ ] Зафиксировать `0 = NO_CLASS`.
...
**Важно:** не уменьшать поле с 4 байт до 1 байта до фиксации этого решения в спецификации.

## 3.2. Data Node

- [ ] Определить окончательную структуру Data Node.
- [ ] Определить размер каждого поля.
- [ ] Убрать DT G1-specific поля, если они больше не нужны.
- [ ] Синхронизировать структуру C parser и Python converter.
- [ ] Зафиксировать offsets всех последующих полей.
...
- [ ] Проверить текущую семантику `v3_jump` в существующем traversal.
- [ ] Определить, действительно ли `+8` является только DT G1 prefetch compensation.
- [ ] Убрать зависимость от старой семантики DT G1.
- [ ] Определить окончательную семантику:
  - `v3_jump` = точный физический размер пропускаемого subtree.
- [ ] Зафиксировать единицы измерения — байты.
- [ ] Проверить прямое использование значения через `purrgo_fs_seek()`.
- [ ] Синхронно обновить Python converter и C parser.
- [ ] Добавить validation для всех `v3_jump`.

## 4.2. SQT

- [ ] Определить окончательную структуру SQT block.
- [ ] Удалить неиспользуемые DT G1 поля.
- [ ] Зафиксировать `mode`.
- [ ] Зафиксировать количество root nodes.
- [ ] Зафиксировать структуру нескольких SQT/LOD.
- [ ] Зафиксировать способ поиска нужного SQT для LOD.
...
- [ ] Добавить regression tests для переходов:
  ...
  - `5 km`;
  - `5 km + 1`.
- [ ] Проверить переключение LOD при увеличении и уменьшении масштаба.
...
- [ ] Генерировать новый PurrGO global header.
- [ ] Убрать все зависимости от `32-byte YZL`.
- [ ] Генерировать окончательный Data Node.
- [ ] Генерировать окончательный Nav Node.
- [ ] Генерировать новый `v3_jump`.
- [ ] Корректно рассчитывать все offsets.
- [ ] Корректно рассчитывать `v1` относительно нового header.
- [ ] Генерировать окончательную структуру SQT/LOD.

## C parser

- [ ] Читать новый PurrGO header.
- [ ] Проверять сигнатуру.
- [ ] Проверять версию.
- [ ] Использовать offset первого SQT вместо hardcoded `32`.
- [ ] Читать окончательный Data Node.
- [ ] Читать окончательный Nav Node.
- [ ] Использовать новую семантику `v3_jump`.
- [ ] Удалить все скрытые DT G1 assumptions.
...
- [ ] Проверить соответствие интерпретации Python converter и C parser.
- [ ] Добавить автоматические regression tests.
- [ ] Проверять старые тестовые карты после каждого изменения формата.
- [ ] Добавить тесты на некорректный header.
- [ ] Добавить тесты на неизвестную версию.
- [ ] Добавить тесты на повреждённые offsets.
...
- [ ] Убрать оставшиеся DT G1-specific assumptions.

---

# 9. P1 — PC reference renderer

До появления STM32 создать возможность полноценно тестировать map path на PC.

- [ ] Запускать C map parser на PC.
- [ ] Загружать эталонные `.idx/.mlp`.
- [ ] Выполнять тот же traversal, что и на STM32.
- [ ] Выполнять integer projection.
- [ ] Выполнять culling.
- [ ] Выполнять renderer.
- [ ] Сохранять результат для визуального regression testing.
- [ ] Сравнивать результаты после изменений parser/format/converter.
...
- [ ] Вынести параметры styles в единую конфигурацию при появлении нескольких renderer.
...
- [ ] Определить физическое представление POI в `.mlp`.
...
- [ ] Перерисовывать карту только при изменении camera/scale.
- [ ] Перерисовывать marker только при изменении позиции/курса.
- [ ] Перерисовывать UI только при изменении данных.
- [ ] Исключить постоянный framebuffer refresh.
...
- Не использовать DT G1-specific semantics без документированной необходимости.
...
1. [ ] PurrGO global header.
2. [ ] Финальная структура Data Node.
3. [ ] Финальная структура Nav Node.
4. [ ] Финальная семантика `v3_jump`.
5. [ ] Финальная структура SQT.
6. [ ] Финальный feature code.
7. [ ] Обновление Python converter.
8. [ ] Обновление C parser.
